...
**Getting Started guide (Stage 6A5).** A second Help sub-view, `#getting-started`, gives newcomers a mobile-first orientation to what the hobby actually involves beyond the exam — Parks on the Air and other outdoor/portable operation, hiking/camping/mobile radio, satellite and ISS contacts, digital modes and experimentation, emergency/public-service communication, and home stations/clubs/mentors — ending with a "Learn more" section of HTTPS links (FCC, ARRL, POTA, ARISS, AMSAT) and the same listening-never-authorizes-transmitting distinction as the SDR paragraph above. `#help-newcomer` gained a short intro sentence and an `Open Getting Started` button near the top of Help, per the design goal of keeping Help itself concise by linking to the guide rather than inlining all of its content there. The button (review follow-up) sits in its own `<section id="guide-cta">` wrapper with a one-line supporting label ("New here?"), separating it from the surrounding paragraph text so it reads as an obviously clickable call to action rather than an inline link — the original inline-button layout was too easy to miss. `#guide-cta` deliberately reuses the descendant selector `.help-content section` (not a `>`-scoped child selector) already applied to every other Help sub-section, so the CTA inherits that rule's panel background/border/radius/padding/shadow for free; an ID-selector override, `#guide-cta { background: var(--choice-bg); border-color: var(--accent); }`, wins outright on specificity alone (an ID beats a class+type combination regardless of source order), changing only the background and border color to the theme's `--choice-bg`/`--accent` tokens so it reads as distinct from the plain-bordered boxes around it without duplicating the shared panel styling. `#openGettingStarted` shares its background/foreground/border-color/weight with the existing `.figure-viewer-btn[aria-pressed="true"]` rule (one review-follow-up merge: the two were declaring identical `--accent`/`--on-accent`/`font-weight:800` values separately) and gets `width: 100%` inside the existing 640px mobile breakpoint, so it is nearly full-width on compact screens and natural-width on larger ones without a new media query. The guide shares Help's overlay mechanics rather than duplicating them: `helpView` (`null` | `"help"` | `"guide"`) tracks which of the two sub-view elements (`#help` / `#getting-started`, both `.help-panel`) is unhidden; entering the overlay from study mode — closing the drawer/figure viewer, pausing a running countdown, hiding the study shell — happens once, in `enterHelpOverlay()`, regardless of which sub-view is entered first or how many times the two are switched between afterward, so switching between them never re-pauses an already-paused timer or re-hides an already-hidden shell. Opening the guide (from Help's button, or a direct `#getting-started` deep link) pushes a new history entry, matching how opening Help itself behaves; stepping back to Help — the guide's `Back to Help` button or `Escape` — instead *replaces* the current history entry with `#help`, so the browser's own Back button moves on to study next rather than bouncing forward into the guide just left (`backToHelp()`, mirroring `closeHelp()`'s existing use of `replaceState` for the same reason). The guide's one photo (`assets/portable-radio-outdoors.jpg`, a portable/POTA-style setup) is inlined as a `data:` URI by `scripts/build.js` exactly like every other embedded asset — resized, compressed, and stripped of all EXIF/GPS/timestamp/Motion-Photo metadata before being committed — with no manifest of its own, unlike the question-figure pipeline below: it is a single static asset with nothing to validate it against, not one tied to per-question data.

**Newcomer onboarding clarity (review follow-up).** The guide's opening paragraph states plainly what the app is ("this is a US FCC Amateur Radio exam study app") and explains the three license classes in one sentence each — Technician the usual starting point, General broader privileges after Technician, Extra the highest class — before the existing jargon-light "What people do with it" list (POTA, hiking/camping/mobile, satellites/ISS, digital modes, emergency/public-service communication, clubs/mentors/home stations), which is unchanged. The "A simple path" section states the path as `Learn → Practice → Take the exam → Explore the hobby` and adds one action, `#startTechnician` ("Start with Technician") — a plain `<button>` styled entirely by the existing global `button` rule (no new CSS). Its handler is three existing calls plus the guide's own `closeHelp()`, in sequence: `setPool("technician")` (which already resets Study scope to `"all"` and restores that pool's saved position, or falls back to the first question, exactly as the `#pool` selector's own `onchange` handler does), `showQuestion()` (renders the now-current Technician question before the overlay closes), `persistState()` (saves the new active-pool choice), then `closeHelp()` (unchanged — hides the overlay, restores focus to `#menuButton`, and clears the URL hash, identically to every other way Help/the guide is dismissed). No new function, storage key, or schema field was added; Mock Exam, the guide's history/Escape/Back/deep-link behavior, and every other pool's own saved state and every preference (theme, Study order, recall delay, exam-timer) are untouched by construction, since `setPool()` only ever reads/writes the target pool's own entry in `appState.study.pools`.
