...
3. `convert -colorspace Gray` — **every figure is monochrome black line art**
   ...
   Technician and General images it is a no-op.
4. `optipng -o5 -strip all` — lossless re-compression, metadata stripped.

Output: `assets/figures/<pool>/<id>.png`, 8-bit grayscale, non-interlaced, chunks
limited to `IHDR`/`IDAT`/`IEND`.
...
- Source: `technician-2026-2030`, **PDF page 78** (image XObject 155, 1800×1200 @ 300 ppi). Asset 29,567 B, 1800×1200, grayscale.
...
- Source: `technician-2026-2030`, **PDF page 79** (image XObject 158, 1800×1200 @ 300 ppi). Asset 36,920 B, 1800×1200, grayscale.
...
- Source: `technician-2026-2030`, **PDF page 79** (image XObject 159, 1800×1200 @ 300 ppi). Asset 22,073 B, 1800×1200, grayscale.
- Extraction: raster-export (already grayscale).
...
- Source: `general-2023-2027`, **PDF page 87** (image XObject 175 + fully-opaque smask, 1845×1455 @ 264 ppi). Asset 59,893 B, 1845×1455, grayscale.
...
- Source: `extra-2024-2028`, **PDF page 119**, image XObject 238 (817×805 @ 264 ppi). Asset 22,225 B, 817×805, grayscale.
...
- Source: `extra-2024-2028`, **PDF page 119**, image XObject 240 (801×570 @ 258 ppi). Asset 15,727 B, 801×570, grayscale.
...
- Source: `extra-2024-2028`, **PDF page 119**, image XObject 242 (826×540 @ 267 ppi). Asset 14,110 B, 826×540, grayscale.
- Extraction: raster-export (→ grayscale).
...
- Source: `extra-2024-2028`, **PDF page 119**, image XObject 244 (790×533 @ 255 ppi). Asset 9,642 B, 790×533, grayscale.
- Extraction: raster-export (→ grayscale).
...
- Source: `extra-2024-2028`, **PDF page 120**, image XObject 249 (796×674 @ 257 ppi). Asset 14,034 B, 796×674, grayscale.
- Extraction: raster-export (→ grayscale).
...
- Source: `extra-2024-2028`, **PDF page 120**, image XObject 251 (782×657 @ 252 ppi). Asset 15,751 B, 782×657, grayscale.
- Extraction: raster-export (→ grayscale).
...
- Source: `extra-2024-2028`, **PDF page 120**, image XObject 253 (738×614 @ 238 ppi). Asset 9,754 B, 738×614, grayscale.
- Extraction: raster-export (→ grayscale).
...
- Source: `extra-2024-2028`, **PDF page 120**, image XObject 255 (759×649 @ 245 ppi). Asset 32,946 B, 759×649, grayscale.
- Extraction: raster-export (→ grayscale).
...
- Source: `extra-2024-2028`, **PDF page 121**, image XObject 259 (760×581 @ 245 ppi). Asset 30,632 B, 760×581, grayscale.
- Extraction: raster-export (→ grayscale).
...
- Source: `extra-2024-2028`, **PDF page 121**, image XObject 261 (395×366 @ 126 ppi). Asset 12,653 B, 395×366, grayscale.
...
restricted SVG subset is not designed for automated PDF→SVG export, but that is
by design; since all 14 sources are raster, `validatePng` (colour type 0,
bit depth 8, chunks `IHDR`/`IDAT`/`IEND`) accepts every asset. `raster-export`
is the recorded `extractionMethod` for all 14; none require a `review` block
(only `hand-tracing` does).
...
Per-asset and aggregate sizes of the committed 8-bit grayscale PNGs:

| Figure | Bytes | Figure | Bytes | Figure | Bytes |
|---|--:|---|--:|---|--:|
| T-1 | 29,567 | E6-1 | 15,727 | E7-3 | 9,754 |
| T-2 | 36,920 | E6-2 | 14,110 | E9-1 | 32,946 |
| T-3 | 22,073 | E6-3 | 9,642 | E9-2 | 30,632 |
| G7-1 | 59,893 | E7-1 | 14,034 | E9-3 | 12,653 |
| E5-1 | 22,225 | E7-2 | 15,751 | | |
| **Aggregate** | | | | **325,927 B** (**318.3 KiB**) |

Largest single asset: G7-1, 59,893 B — well under the 512 KiB per-PNG contract
limit; every asset is under 60 KiB.

**Standalone embedding estimate (assumptions stated).** Stage 3 is not built, so
this is an estimate, **not** a compliance claim.

- Standalone budget: **1,048,576 B** (1 MiB), enforced on `dist/index.html` by
  the future packaging stage.
- Current `dist/index.html`: **633,892 B** → headroom today **414,684 B**.
- If the 14 PNGs are inlined as `data:image/png;base64,…` URIs (as the app
  already does for its icons): 325,927 × 4/3 ≈ **434,570 B**, plus ~30 B of
  URI/attribute wrapper per figure ≈ **~435 KB**. That **exceeds today's
  headroom by ~20 KB before any Stage 3 UI/CSS/JS is added** — inlining the
  assets *as-is* does **not** fit the standalone budget.
- Mitigations for Stage 2D / Stage 3 to choose between (do not decide here):
  1. Re-encode the line art to **1-bit / indexed PNG** (the validator already
     accepts colour type 0 bit depth 1 and colour type 3). A 50 %-threshold
     bilevel re-encode of the same crops measured **71,160 B aggregate**
     (~95 KB base64) — comfortably within budget, at the cost of anti-aliasing
     (most visible on E9-3 and the two antenna-pattern curves).
  2. **Precache the figures as separate PWA files** instead of inlining them
     into the standalone shell (already an open decision point in the plan).
  3. A hybrid: 1-bit inline for the standalone file, full-grayscale precached
     copies for the PWA.

Stage 2C keeps the **higher-quality 8-bit grayscale** masters; the size/format
trade-off is a packaging decision.

---

## 7. Review status

| Figure | Agent structural check | Agent visual inspection | **Human fidelity review** |
|---|---|---|---|
| T-1 | pass | pass | **PENDING** |
| T-2 | pass | pass | **PENDING** |
| T-3 | pass | pass | **PENDING** |
| G7-1 | pass | pass | **PENDING** |
| E5-1 | pass | pass | **PENDING** |
| E6-1 | pass | pass (fine detail near source limit) | **PENDING** |
| E6-2 | pass | pass | **PENDING** |
| E6-3 | pass | pass | **PENDING** |
| E7-1 | pass | pass | **PENDING** |
| E7-2 | pass | pass | **PENDING** |
| E7-3 | pass | pass | **PENDING** |
| E9-1 | pass | pass | **PENDING** |
| E9-2 | pass | pass (confirm on-figure title in alt) | **PENDING** |
| E9-3 | pass | pass (low source resolution) | **PENDING** |

A human reviewer should, for each figure: open the source PDF at the recorded
page, place the asset beside it, and confirm every label / number / connection /
symbol / axis / orientation is present and legible with nothing added or
removed; confirm the alt text reveals nothing for any listed question; and sign
and date this table. Until then the figure pipeline is **not** cleared for
Stage 3 rendering.
