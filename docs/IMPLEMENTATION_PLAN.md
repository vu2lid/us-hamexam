...
Last reviewed: September 19, 2026 (beta.4 release preparation).

**Current application priority:** Complete independent review of the uncommitted
beta.4 work and resolve the standalone artifact's very low remaining size margin
...
checks remain a separate, still-open track. **Stage 5 (beta.2 completeness) is complete and released**: 5A (metadata/exam-config consolidation, `980c2a0`), 5B1 (descriptions and semantic-version-derived release labels, `559bf38`), 5B2 (pull-request CI and generated-artifact freshness, `5c5fe45`), 5B3 (documentation/test-inventory reconciliation, `c65cffc`), and 5B4 (question-bank schema validation, `27af87c`) are complete. The beta.2 package was released; its final-gate and physical-device notes remain historical context. **Stage 6A (transient scoped study navigation)** — the first Stage 6 item, done as a study-mode-only, non-persisted feature per [`docs/SCOPED_STUDY_PLAN.md`](SCOPED_STUDY_PLAN.md) — **is complete and released as part of `0.3.0-beta.3`** (merged via PR #3); see the Stage 6A, Stage 6A-verification, and Stage 6A review-fix execution-log rows below. **Stage 6A1 (human-readable subelement/group titles in the scope selector, sourced from the tracked NCVEC pool text)** — is complete, reviewed, and committed as `3df1252`; see its execution-log rows below and [`docs/SCOPED_STUDY_PLAN.md`](SCOPED_STUDY_PLAN.md#stage-6a1-human-readable-scope-labels). **Stage 6A3 (pause the study recall timer while the Settings drawer is open)** — unrelated to scoped study, built on the same `main` tree — is complete, reviewed, and committed locally as `879f37e` (**not yet pushed**; public release remains `0.3.0-beta.3`); see its execution-log rows below and [`docs/RESPONSIVE_LAYOUT_PLAN.md`](RESPONSIVE_LAYOUT_PLAN.md). **Stage 6A4 (Help & About audit, "New to Amateur Radio?" newcomer guidance, and `0.3.0-beta.4` release preparation)** — bundling Stage 6A1, 6A3, and this stage's own content as the beta.4 release — **is implemented, uncommitted, pending independent review**, and has already gone through one review-driven correction (replacing two HTTP-only receiver-directory links with HTTPS educational SDR resources); see its execution-log rows below and `docs/RELEASE_NOTES_0.3.0-beta.4.md`. **The current application priority is completing independent review of this uncommitted beta.4 work and resolving the standalone artifact's very low remaining size margin (see the most recent Stage 6A4 execution-log row for the exact figure) before any further feature work; Stage 6B (persisted scope) remains explicitly deferred until both are resolved.**
...
- [x] Stage 6A4: Help & About audit, "New to Amateur Radio?" newcomer
  guidance, and `0.3.0-beta.4` release preparation — implemented,
  uncommitted, pending independent review; see the Stage 6A4 execution-log
  row below.
