...
checks remain a separate, still-open track. **Stage 5 (beta.2 completeness) is in final release preparation**: 5A (metadata/exam-config consolidation, `980c2a0`), 5B1 (descriptions and semantic-version-derived release labels, `559bf38`), 5B2 (pull-request CI and generated-artifact freshness, `5c5fe45`), 5B3 (documentation/test-inventory reconciliation, `c65cffc`), and 5B4 (question-bank schema validation, `27af87c`) are complete. The package is prepared as `0.3.0-beta.2`; the remaining work is the final release gate and explicit physical-device checks. **Stage 6A (transient scoped study navigation)** — the first Stage 6 item, done as a study-mode-only, non-persisted feature per [`docs/SCOPED_STUDY_PLAN.md`](SCOPED_STUDY_PLAN.md) — **is implemented and independently verified (unit, standalone-determinism, `@compat`/`@responsive`/PWA browser suites, and a size audit), uncommitted, pending independent review**; see the Stage 6A and Stage 6A-verification execution-log rows below. **It targets the next feature release (0.4.0, or a later beta) — not the already-published `0.3.0-beta.2`**, which remains released and unaffected; the final release gate and explicit physical-device checks for that next release are a separate, still-open step from Stage 6A itself. Stage 6B (persisted scope, deferred) remains open.
