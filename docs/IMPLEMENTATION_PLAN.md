...
Plan status: Stage 1 complete and verified. Stage 2 nearly complete — 2A
(mappings), 2B (validator), 2C (real manifest + assets), and 2D (mandatory
build gate) all done; the only remaining Stage 2 item is the per-figure **human**
fidelity sign-off (`docs/FIGURE_REVIEW.md` §7). Stage 4 may proceed
independently.

Current stage: Stage 2 (figure pipeline) — closing. Next: obtain the human
fidelity review, then Stage 3 (rendering / packaging).
...
- [~] Enforce a complete standalone size budget of at most 1 MiB. _(Stage 2B:
  ...
  code — see `docs/FIGURE_REVIEW.md` §6. **Figure-optimisation experiment
  (`docs/FIGURE_OPTIMIZATION.md`, `scripts/figure-optim-experiment.js`):** a
  16-level grayscale re-encode (`-depth 4`, no dither) of 13 figures + E5-1 kept
  at 8-bit → 306,008 B base64 → estimated `dist/index.html` ≈ 943,208 B, ≈
  39,832 B free after a 64 KiB Stage 3 allowance; all candidates pass
  `validatePng`; agent visually compared all 14 figures and found `g16`
  indistinguishable from the 8-bit baseline on the 13 recommended ones (the
  pixel metrics are threshold-vacuous at `maxΔ ≤ 16`, so the visual review is
  the evidence; human sign-off still pending). No production asset changed.
  Final embedded-size enforcement belongs to Stage 3 packaging.)_
